...
def checkout(path, repository, revision):
    """Create an immutable dependency checkout, refusing to overwrite another revision."""
    ...
    if actual != revision:
        raise RuntimeError(f"{path}: expected {revision}, found {actual}; use a fresh build directory")


def main():
    """Populate build-owned dependency sources without changing either application's checkout."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("source", type=pathlib.Path)
    args = parser.parse_args()
    checkout(args.source, "https://github.com/Themaister/pyrowave.git", PYROWAVE_REVISION)
    granite = args.source / "Granite"
    checkout(granite, "https://github.com/Themaister/Granite.git", GRANITE_REVISION)
    subprocess.run(["git", "-C", str(granite), "submodule", "update", "--init", "--depth", "1",
